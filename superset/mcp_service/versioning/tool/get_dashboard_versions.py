...
# under the License.
"""Get dashboard version history FastMCP tool."""
...
async def get_dashboard_versions(
    ...
    """Version history of a dashboard, or one full version snapshot.

    Same entries as ``GET /api/v1/dashboard/<uuid>/versions/`` and
    ...
    - With ``version_uuid``: the dashboard's state at that version plus the
    ...
    Access: requires read access to the dashboard itself (fail-closed gate
    shared with the REST endpoints). Not exposed to guest tokens.
    """
    from superset.models.dashboard import Dashboard

    return await versions_tool_body("dashboard", Dashboard, request, ctx)
