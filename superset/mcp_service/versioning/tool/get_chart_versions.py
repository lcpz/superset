...
# under the License.
"""Get chart version history FastMCP tool."""
...
async def get_chart_versions(
    ...
    """Version history of a chart, or one full version snapshot.

    Same entries as ``GET /api/v1/chart/<uuid>/versions/`` and
    ...
    - With ``version_uuid``: the chart's state at that version plus the
    ...
    Access: requires read access to the chart itself (fail-closed gate
    shared with the REST endpoints). Not exposed to guest tokens.
    """
    from superset.models.slice import Slice

    return await versions_tool_body("chart", Slice, request, ctx)
