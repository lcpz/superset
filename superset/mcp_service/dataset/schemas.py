...
# ---- Dataset migration evidence export -------------------------------------


class ExportDatasetMigrationEvidenceRequest(BaseModel):
    """Request for a bounded, SHA-256 digested migration evidence page."""
    ...
    dataset_uuid: Annotated[
        str,
        Field(
            description="UUID of the dataset being retired/migrated",
            validation_alias=AliasChoices("dataset_uuid", "uuid", "identifier"),
        ),
    ]
    since: Annotated[
        ...
        Field(
            None,
            description=(
                "ISO-8601 start of the review window (UTC). Versions issued "
                "before it define the `before` snapshot."
            ),
        ),
    ] = None
    until: Annotated[
        ...
        Field(
            None,
            description="ISO-8601 end of the review window (UTC); default: now",
        ),
    ] = None
    page: Annotated[int, Field(0, ge=0, description="0-based inventory page")] = 0
    page_size: Annotated[
        int,
        Field(
            10,
            ...
            le=25,
            description=(
                "Dependent charts/dashboards per page (each asset adds versions, "
                "two snapshots and activity to the bundle; keep small)"
            ),
        ),
    ] = 10
    record_limit: Annotated[
        ...
        Field(
            ...
            ge=1,
            le=200,
            description="Hard cap per record collection (versions/activity/logs)",
        ),
    ] = 200


class EvidenceDigest(BaseModel):
    algorithm: str = Field(..., description="Always 'sha256'")
    value: str = Field(..., description="Hex digest of the canonical evidence dict")
    covers: str = Field(
        ...,
        description=(
            "Which part of the response is hashed ('evidence'); recompute with "
            "hash_from_dict(evidence, algorithm='sha256') to verify"
        ),
    )


class DatasetMigrationEvidenceResponse(BaseModel):
    """Same body as GET /api/v1/dataset/<uuid>/migration_evidence/."""

    evidence: dict[str, Any] = Field(
        ...,
        description=(
            "Canonical evidence bundle: dataset, window, page, inventory, "
            "assets[] (versions_in_window, before, after, activity), "
            "report_executions, query_executions, coverage"
        ),
    )
    digest: EvidenceDigest
    generated_at: str = Field(
        ..., description="Wall-clock generation time; NOT part of the digest"
    )
