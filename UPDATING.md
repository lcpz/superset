...
The watermark is stored in the `key_value` metastore table (under `SharedKey.VERSION_HISTORY_PRUNE_WATERMARK`) and updated by each prune run; no migration or operator action is required. The watermark write is **not atomic** with the prune's `DELETE` (it runs on `db.session` after the SERIALIZABLE prune passes commit, since it cannot join the raw prune connection), and it uses an unlocked read-then-write update. A crash between the commit and the write, or a concurrent prune run, can therefore leave the watermark lagging the last deletion by up to one run. This is fail-safe rather than incorrect: `history_begins_at` is the later of the watermark and the current-policy cutoff, so a lagging watermark only means the cutoff dominates — the disclosed floor stays conservative and never falsely claims completeness — and the next successful prune recomputes and advances the watermark. Beat schedules one prune at a time by default; hardening this into a database-atomic monotonic update is a possible follow-up.
